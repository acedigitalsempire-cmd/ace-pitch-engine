...
function drawCover(doc, lead, W, H) {
  ...
  doc.font('Helvetica').fontSize(9)
    .text('acedigitalsglobal@gmail.com  |  acedigitalsempire.com  |  WhatsApp: +1 (873) 352-2008', 50, H - 58);
...
function drawFooter(doc, W, H, pageNum) {
  ...
  doc.fillColor(hex2rgb(BRAND.navy))
    .font('Helvetica').fontSize(8)
    .text('Ace Digitals Global  |  acedigitalsglobal@gmail.com  |  acedigitalsempire.com  |  WhatsApp: +1 (873) 352-2008',
...
function drawRevenueImpact(doc, lead, W, H) {
  ...
  // Comparison block — use solid light backgrounds so text is always visible
  const boxW   = (W - 90) / 2;
  const col2x  = 40 + boxW + 10;
  const boxH   = 110;

  // LEFT box: light pink background, dark red border
  doc.rect(40, y, boxW, boxH).fill([255, 235, 235]);
  doc.rect(40, y, boxW, boxH).stroke(hex2rgb(BRAND.red));

  // RIGHT box: light green background, dark green border
  ...
  // RIGHT list — dark slate so it is always readable
  const withList = [
    'Ranking on Google for local searches',
    'Leads and quote requests 24/7',
    'Professional credibility online',
    'Portfolio and reviews visible',
  ];
  withList.forEach((w, i) => {
    doc.fillColor(hex2rgb(BRAND.navy)).font('Helvetica').fontSize(9)
      .text(`+  ${w}`, col2x + 10, y + 28 + i * 18, { width: boxW - 20 });
  });
}
...
function drawCTAPage(doc, lead, W, H) {
  ...
  doc.fillColor(hex2rgb(BRAND.navy)).font('Helvetica').fontSize(10)
    .text(`Email:      acedigitalsglobal@gmail.com`, 60, sy + 46);
  doc.text(`WhatsApp: +1 (873) 352-2008`, 60, sy + 64);
  doc.text(`Website:    acedigitalsempire.com`, 60, sy + 80);

  doc.fillColor(hex2rgb(BRAND.navy)).opacity(0.6).font('Helvetica').fontSize(8)
    .text('(WhatsApp preferred for fastest response)', 60, sy + 100);
