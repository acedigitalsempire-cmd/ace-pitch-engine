...
// ─── GMAIL SMTP ───────────────────────────────────────────────────────────────
// Uses Gmail App Password — port 465 (SSL) works on Render free tier
...
function getGmailTransport() {
  if (!_gmailTransport) {
    _gmailTransport = nodemailer.createTransport({
      ...
      secure: true,
      auth: {
        user: 'acedigitalsglobal@gmail.com',
    ...
    _gmailTransport.verify((err) => {
      if (err) console.error('[Gmail] SMTP connection failed:', err.message);
...
async function sendEmail({ from, to, subject, html, attachments, headers }) {
  ...
  console.log('[Mailer] Resend not configured — using Gmail SMTP');
...
router.post('/', async (req, res) => {
  ...
  if (!resendKey && !gmailPass)
    return res.status(500).json({ error: 'No email provider configured. Set RESEND_API_KEY or GMAIL_APP_PASSWORD.' });
