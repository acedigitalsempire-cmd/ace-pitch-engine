/**
 * /api/send        — Email delivery via Gmail HTTP API (primary) or Resend (if domain active)
 * /api/preview-pdf — Returns PDF buffer for browser preview
 */
...
async function sendViaGmail({ to, subject, html, attachments }) {
  ...
  const fromEmail = 'acedigitalsglobal@gmail.com';
  ...
  const encodedMessage = Buffer.from(raw.join('\r\n'))
    .toString('base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=+$/, '');
...
async function sendEmail({ to, subject, html, attachments }) {
  ...
  if (useResend) {
    ...
    const result = await resendPost({
      from:        `Richard U. | Ace Digitals Global <${process.env.RESEND_FROM_EMAIL}>`,
      to:          Array.isArray(to) ? to : [to],
      subject,
      html,
      attachments,
      headers: { 'List-Unsubscribe': `<mailto:${process.env.RESEND_FROM_EMAIL}?subject=Unsubscribe>` },
    });
  ...
  return await sendViaGmail({ to, subject, html, attachments });
}
...
router.post('/', async (req, res) => {
  ...
  const ownerEmail = process.env.OWNER_EMAIL || 'acedigitalsglobal@gmail.com';
  ...
  try {
    ...
    await sendEmail({
      ...
      html: `<div style="font-family:Arial,sans-serif;padding:20px;max-width:620px">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:20px">
          ...
          <tr><td style="padding:10px;color:#718096;border:1px solid #E5E7EB">Subject sent</td><td style="padding:10px;border:1px solid #E5E7EB">${subject}</td></tr>
        </table>
        <h3 style="color:#1B2A4A;margin-bottom:10px">Pitch sent:</h3>
    ...
    console.log(`[Send] Done for ${lead.name}`);
    return res.json({ success: true, to: lead.email });
...
<div style="max-width:620px;margin:28px auto;background:#fff;border-radius:4px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08)">
  ...
  <div style="padding:32px 36px">${bodyLines}</div>
  ...
  <div style="margin:0 36px 24px;background:#FEF9EC;border-left:3px solid #C9A84C;padding:14px 16px;border-radius:2px">
    ...
    <div style="font-size:12px;color:#78350F;line-height:1.5">We put together a personalised report for ${lead.name} showing exactly which customers are slipping through and a clear plan to fix it.</div>
  </div>
  <div style="background:#F8F9FC;border-top:1px solid #E5E7EB;padding:18px 36px">
    ...
    <div style="margin-top:12px;font-size:11px;color:#9CA3AF;line-height:1.6">
      You received this email because ${lead.name} appeared in public business directories without a website.<br>
