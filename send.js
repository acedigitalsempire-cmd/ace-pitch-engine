/**
 * /api/send   — Email delivery via Resend (primary) or Brevo SMTP (fallback)
...
// ─── BREVO SMTP ───────────────────────────────────────────────────────────────
// Created once at startup — avoids cold connection delay on first send
...
function getBrevoTransport() {
  if (!_brevoTransport) {
    ...
    _brevoTransport.verify((err) => {
      if (err) console.error('[Brevo] SMTP connection failed:', err.message);
...
// ─── UNIFIED SEND ─────────────────────────────────────────────────────────────
async function sendEmail({ from, to, subject, html, attachments, headers }) {
  const resendKey = process.env.RESEND_API_KEY;
  const useResend = resendKey && process.env.RESEND_FROM_EMAIL;
  ...
  console.log('[Mailer] Resend not configured — using Brevo SMTP');
  ...
  return { status: 200, body: { id: info.messageId } };
}
...
router.post('/', async (req, res) => {
  ...
  if (!resendKey && (!brevoUser || !brevoPass))
    return res.status(500).json({ error: 'No email provider configured. Set RESEND_API_KEY or BREVO_SMTP_USER + BREVO_SMTP_PASS.' });

  const fromEmail  = process.env.RESEND_FROM_EMAIL || process.env.BREVO_SMTP_USER;
